...
def diretorio_dados():
    """Retorna uma pasta gravável, fora do bundle do PyInstaller."""
    if os.name == "nt":
        base = os.getenv("LOCALAPPDATA") or os.getenv("APPDATA")
        if base:
            return Path(base) / "Neymar IA" / "data"
    return Path.home() / ".neymar_ia" / "data"


def caminho_asset(nome):
    """Localiza assets tanto no código-fonte quanto no executável."""
    if getattr(sys, "frozen", False):
        base = Path(getattr(sys, "_MEIPASS", Path(sys.executable).parent)) / "neymar_ia" / "assets"
    else:
...
CAMINHO_FC26 = os.getenv(
    "NEYMAR_CAMINHO_FC26",
    r"D:\Steam\steamapps\common\FC 26\FC26.exe"
)
